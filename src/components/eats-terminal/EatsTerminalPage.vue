<template>
    <div class="subContainer">
            ...
            <div v-if="!isLoading">
                ...
                <span class="pageTitleSwitcher">
                    ...
                    <select v-model="deviceId" class="select"
                        @change="toast.info($t('others.changeTurniket')); loadPerson()">
            ...
            <div class="leftSide">
                ...
                <div class="terminal" v-if="!isLoading">
                    <div class="treminalLeftSide">
                        ...
                        <button class="button red"
                            ...
                            @click="visible = !visible" v-if="scanMode == 'auto'">{{ $t('eatsTerminalLayout.stopScan')
                            }}</button>

                        <button class="button"
                            style="width: 100%; margin-top: 30px; padding: 15px 10px; white-space: nowrap; ;justify-content: center"
                            v-if="scanMode == 'manual'" @click="visible = !visible">{{
                ...
                isPersonLoaded ? (
                    "биометрия/карта"
                        ...
                        <div class="terminalBtns" :class="!showButtons && 'hide'"
                            v-if="scanMode == 'manual' && isPersonLoaded">
                            <button class="button" style="margin-right: 15px;" @click="personAcceptDecline(1, true)">{{
                $t('tables.accept') }}</button>
                            <button class="button red" @click="personAcceptDecline(0, true)">{{ $t('tables.decline')
                                }}</button>
                        </div>
                    </div>
                </div>
                ...
                <div class="lasts">
                    <EatsTerminalLasts :lasts="lasts" :currentGroup="currentGroup" :loadLastsByPage="loadLastsByPage"
                        :loadLast="loadLasts" />
                </div>
            </div>

            <div class="rightSide">
                <EatsTerminalList v-if="listMode == 'enter'" :listMode="listMode" :listCount="list.total" :list="list"
                    :listOtsut="listOtsut" :currentGroup="currentGroup" :loadListByPage="loadListByPage"
                    :changeListMode="changeListMode" />
                <EatsTerminalList v-if="listMode == 'otsut'" :listMode="listMode" :listCount="list.total"
                    :list="listOtsut" :listOtsut="listOtsut" :currentGroup="currentGroup"
                    :loadListByPage="loadListOtsutByPage" :changeListMode="changeListMode" />
            </div>
        </div>
        ...
        <Dialog v-model:visible="visible" modal :header="$t('eatsGroupsLayout.sure')" :style="{ width: '25vw' }"
            ...
            <div class="footer">
                <button class="button" @click="changeScanMode()" style="margin-left: 10px;">{{
                    $t('eatsGroupsLayout.yes') }}</button>
...
import { Ref, ref, onBeforeMount, watch, computed, onBeforeUnmount } from 'vue';
...
let awaitingLoadPerson: Ref<boolean> = ref(false)
let lastPersonId: Ref<string> = ref('')

const loadPerson = async () => {
    console.log("awaitingLoadPerson.value: " + awaitingLoadPerson.value)
    if (awaitingLoadPerson.value) {
        return
    }
    awaitingLoadPerson.value = true
    await eatsGroupStore.fetchVerifPerson(deviceId.value.toString()).then(async () => {
        ...
        } else if (scanMode.value == "auto" && lastPersonId.value != person.value[0].id) {
            await personAcceptDecline(1, false)
            lastPersonId.value = person.value[0].id
        }

        showButtons.value = true;
    }).finally(() => {
        awaitingLoadPerson.value = false
        console.log("end")
    })
...
watch(scanMode, (newValue, oldValue) => {
    ...
    if (newValue === 'auto' && intervalId === null) {
        ...
        }, 1000); // Интервал в миллисекундах, например, 5000 мс (5 секунд)
