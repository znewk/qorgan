<template>
    <div class="subContainer">
        <div class="pageTitle">
            <span class="pageTitleValues">{{$t('categoriesLayout.title')}}</span>
        </div>
        ...
        <div class="importSide">
            ...
            <button class="button red" @click="deleteSelectedCategories" style="margin-left: 10px;">
        ...
        <Dialog v-model:visible="visible" modal :header="$t('categoriesLayout.createCategory')" :style="{ width: '25vw' }" :draggable="false">
            <div class="inputs">
                <div class="item">
        ...
        <Dialog v-model:visible="visibleEdit" modal :header="$t('categoriesLayout.editCategory')" :style="{ width: '25vw' }" :draggable="false">
            <div class="inputs">
                <div class="item">
                    <span class="itemTitle">
                        {{$t('tables.nameOf')}} <span class="blue">*</span>
            ...
            <div class="footer">
                ...
                <button class="button" @click="sendEditCategory" style="margin-left: 10px;">{{$t('tables.save')}}</button> 
            </div>
        </Dialog>
    </div>
</template>
...
import { Category } from '@/models/books';
...
const selectedCategories: Ref<Category[] | null> = ref(null)
...
const deleteSelectedCategories = async () => {
  if(selectedCategories.value?.length! <= 0 || selectedCategories.value == null){
    toast.warning(t('others.notSelected'))
  } else {
    const selectedIds = selectedCategories.value.map(category => category.id)

    await categoriesStore.fetchDeleteCategory(selectedIds)
    await loadCategories()
