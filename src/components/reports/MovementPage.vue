<template>
    <div class="subContainer">
        <div class="pageTitle">
            <span class="pageTitleValues">{{$t('movementLayout.title')}}</span>
        </div>

        <div class="importSide">
            <a target="_blank" href="/export/excel?viewName=v_lib_report2" style="text-decoration: none;">
                <button class="button" >
        ...
        <div class="tableBody">
            ...
            >
                ...
                <Column field="division" :header="$t('tables.otdel')" sortable style="min-width: 12rem">
                    <template #body="{ data }">
                        {{ data.division }}
                    </template>
                    <template #filter="{ filterModel, filterCallback }">
                        <InputText v-model="filterModel.value" type="text" @input="filterCallback()" class="p-column-filter" :placeholder="$t('tables.otdel')" />
                    </template>
                </Column>
                <Column field="fioSt" :header="$t('tables.fio')" sortable style="min-width: 12rem">
                    <template #body="{ data }">
                        {{ data.fioSt }}
                    </template>
                    <template #filter="{ filterModel, filterCallback }">
                        <InputText v-model="filterModel.value" type="text" @input="filterCallback()" class="p-column-filter" :placeholder="$t('tables.fio')" />
                    </template>
                </Column>

                <Column field="issueDate" :header="$t('tables.giveDate')" sortable style="min-width: 12rem">
                    <template #body="{ data }">
                        {{ data.issueDate }}
                    </template>
                    <template #filter="{ filterModel, filterCallback }">
                        <InputText v-model="filterModel.value" type="text" @input="filterCallback()" class="p-column-filter" :placeholder="$t('tables.giveDate')" />
                    </template>
                </Column>

                <Column field="returnDate" :header="$t('tables.returndate')" sortable style="min-width: 12rem">
                    <template #body="{ data }">
                        {{ data.returnDate }}
                    </template>
                    <template #filter="{ filterModel, filterCallback }">
                        <InputText v-model="filterModel.value" type="text" @input="filterCallback()" class="p-column-filter" :placeholder="$t('tables.returndate')" />
                    </template>
                </Column>
                <Column field="author" :header="$t('tables.bibli')" sortable style="min-width: 12rem">
                    <template #body="{ data }">
                        {{ data.author }}
                    </template>
                    <template #filter="{ filterModel, filterCallback }">
...
const report: Ref<MovementReport[] | null> = ref(null)
...
const filters = ref({
    ...
    fioSt: { value: null, matchMode: FilterMatchMode.CONTAINS },
    issueDate: { value: null, matchMode: FilterMatchMode.CONTAINS },
    returnDate: { value: null, matchMode: FilterMatchMode.CONTAINS },
    author: { value: null, matchMode: FilterMatchMode.CONTAINS },
    some: { value: null, matchMode: FilterMatchMode.CONTAINS },
});

const loadReport = async () => {
    await reportsStore.fetchMovement()

    report.value = reportsStore.getMovement!
