<template>
    <div class="container" :style="show ? 'right: 0;' : 'right: -100%;'">
        <div>
            <div class="header">
                <h1 class="title">{{$t('booksLayout.createBook')}}</h1>
            </div>
...
const { 
    show, showHook, loadBooks, publisher, categories, authors, groups, 
...
const onFileUploadComplete = () => {
    toast.success(t('others.uploaded'))
};
...
const create = async () => {
    if (
        ...
        const data = {
            ...
            isbn: isbn.value,
            publisher: {id: selcctedPublishers.value[0].id},
        ...
        try {
            ...
            summary.value = ''
            selcctedPublishers.value = []
            yearPublished.value = null
            isbn.value = null
            barcodeNumber.value = null
            bookCover.value = ''
            description.value = ''
            selectedCategories.value = []
            annotation.value = ''
            totalCopies.value = null
