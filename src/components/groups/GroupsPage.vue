<template>
    <div class="subContainer">
        <div class="pageTitle">
            <span class="pageTitleValues">{{$t('groupsLayout.title')}}</span>
        </div>
        ...
        <div class="importSide">
            ...
            <button class="button red" @click="deleteSelectedGroups" style="margin-left: 10px;">
        ...
        <Dialog v-model:visible="visible" modal :header="$t('groupsLayout.createGroup')" :style="{ width: '25vw' }" :draggable="false">
            <div class="inputs">
                <div class="item">
                ...
                <div class="item">
                    ...
                    <div style="width: 100%;">
                        <InputText style="width: 100%;" type="text" v-model="descriptionInput" :placeholder="$t('tables.description')"/>
                    </div>
        ...
        <Dialog v-model:visible="visibleEdit" modal :header="$t('groupsLayout.editGroup')" :style="{ width: '25vw' }" :draggable="false">
            <div class="inputs">
                <div class="item">
                    <span class="itemTitle">
                        {{$t('tables.nameOf')}} <span class="blue">*</span>
                ...
                <div class="item">
                    ...
                    <div style="width: 100%;">
                        <InputText style="width: 100%;" type="text" v-model="descriptionInputToEdit" :placeholder="$t('tables.description')"/>
                    </div>
            ...
            <div class="footer">
                ...
                <button class="button" @click="sendEditGroup" style="margin-left: 10px;">{{$t('tables.save')}}</button> 
            </div>
        </Dialog>
    </div>
</template>
...
import { Category, Group } from '@/models/books';
...
const groups: Ref<Category[] | null> = ref(null)

const descriptionInput: Ref<string> = ref('')
const titleInput: Ref<string> = ref('')

const descriptionInputToEdit: Ref<string> = ref('')
...
const editReader = (reader: any) => {
    ...
    descriptionInputToEdit.value = reader.description

    visibleEdit.value = true
};

const sendEditGroup= async () => {
    if(titleInputToEdit.value.length == 0 || descriptionInputToEdit.value.length == 0){
        toast.warning(t('others.inputAll'))
    } else {
        const data = {
            id: idToEdit.value,
            description: descriptionInputToEdit.value,
            name: titleInputToEdit.value
        }

        await groupStore.fetachCreateGroup(data)
...
const deleteSelectedGroups = async () => {
  if(selectedGroups.value?.length! <= 0 || selectedGroups.value == null){
    toast.warning(t('others.notSelected'))
  } else {
    const selectedIds = selectedGroups.value.map(group => group.id)
    await groupStore.fetchDeleteGroup(selectedIds)
    await loadGroups()
